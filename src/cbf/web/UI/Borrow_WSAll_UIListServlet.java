...
public class Borrow_WSAll_UIListServlet extends HttpServlet {
...
//		System.out.println("searchBorroeInfo="+search_info);
		...
		if (search_info != null && search_info.length() > 0) {
			...
			select_sql = "select borrowinfo.bid,books.bookname,books.writer,"
						...
						+ " and borrowinfo.uid<>"+user.getUid()+" and (" + whereInfo1 + " or " + whereInfo2
						+ ") order by user.level and borrowinfo.ishistory asc";
		} else {
				select_sql = "select borrowinfo.bid,books.bookname,books.writer,"
						+ "books.publication,borrowinfo.uid,user.name,user.level,"
						+ "borrowinfo.borrowtime,borrowinfo.returntime,borrowinfo.ishistory"
						+ " from books,user,borrowinfo where borrowinfo.uid=user.uid and borrowinfo.bid = books.bid"
						+ " and borrowinfo.uid<>"+user.getUid()+" order by user.level and borrowinfo.ishistory asc";
		}
...
//			request.setAttribute("sError", 0);
		...
		request.getRequestDispatcher("/WEB-INF/jsp/borrowlist_WSALL.jsp").forward(
				request, response);
	}
