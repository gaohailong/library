		...
		<div class="main-content-inner">
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							...
							<li class="active">借阅列表/个人借书列表</li>
						</ul><!-- /.breadcrumb -->
					</div>
		</div>
		<div class="main-content-inner">
	   	 	...
	   	 	<c:when test="${sessionScope.user.level==1}">
		          ...
		          "bSort" : true,//设置自动排序的地方,true表示排序，false表示取消自动排序
								...
								if(!flag)
									if(inhistory == 0)
							   			$('td:eq(6)', nRow).html("<a href='../servlet/BorrowChaKanServlet?chakan1="+chakan1+"&chakan2="+chakan2+"'>查看</a>"
							   				+"/<a href='../servlet/ReturnFunctionServlet?chakan1="+chakan1+"&chakan2="+chakan2+"'>退订</a>");
									else
										$('td:eq(6)', nRow).html("<a href='../servlet/BorrowChaKanServlet?chakan1="+chakan1+"&chakan2="+chakan2+"'>查看</a>");
								else
									$('td:eq(6)', nRow).html("");
						...
						$("div.toolbar").html("<select id='stype' onChange='window.location=this.options[this.selectedIndex].value'>"
								+"<option value='${pageContext.request.contextPath}/servlet/Borrow_WS_UIListServlet' "+
								"onclick='window.location=this.value'>个人借书列表</option>"
								+"<option value='${pageContext.request.contextPath}/servlet/Borrow_WSAll_UIListServlet'>待处理借书列表</option>"
								+"</select>");
						...
						$("#example_filter").html("<form name='search_form' action=../servlet/Borrow_WS_UIListServlet method=post><label><input name='searchBorroeInfo' placeholder='请输入书名或用户名' type='search' class='' style='width:280px' aria-controls='example'><input type='submit' value='搜索' class='search_in btn btn-info btn-xs' name='search'></label></form>");
					});
				</script>
	   		</c:when>
	   		<c:otherwise>
